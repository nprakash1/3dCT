...
Clean grid layout (no overlapping boxes/text).
...
# ================= TITLE =================
ax.text(8, 9.55, "Improved Frozen Model \u2014 CT-CLIP Temporal Difference Transformer",
        ha="center", va="center", fontsize=16, fontweight="bold")
ax.text(8, 9.15, "frozen chest-native CT foundation model  +  one small trainable difference module",
...
box(ix, TY + 1.35, iw, 0.52, "tiny Transformer encoder  (2 layers)", INNER, TRAIN_E, fs=8.6)
box(ix, TY + 0.55, iw, 0.55, "read out e_diff  \u2192  Linear \u2192 512-d", INNER, TRAIN_E, fs=8.6)

arrow((7.4, 7.72), (TX, TY + TH - 0.85), rad=-0.12)
arrow((7.4, 5.92), (TX, TY + TH - 0.85), rad=0.10)

# antisymmetry note (below the box, clear of everything)
ax.text(TX + TW / 2, TY - 0.35,
        "antisymmetry:  d = g(c,p) \u2212 g(p,c)   \u2192   stable = 0 vector",
        ha="center", fontsize=9, style="italic", color="#8a5a12")
...
# optional contrastive (above d)
box(12.15, 8.15, 2.6, 0.72, "InfoNCE(d, evidence)\ncontrastive  (optional)", LOSS, LOSS_E, fs=8.2)
arrow((13.45, 7.85), (13.45, 8.15))

# optional magnitude head (below d)
box(12.5, 5.75, 1.9, 0.72, "||change|| head\n(optional)", INNER, TRAIN_E, fs=8.4)
arrow((TX + TW, TY + 1.0), (12.5, 6.11), rad=0.08)
...
ax.text(0.6, 0.15, "Only the orange module trains (~few M params); both CT-CLIP encoders stay frozen.",
...
print("wrote docs/improved_frozen_model.png")
